...
@export var RUN_MULT: float = 1.3

# Crouch tuning
const CROUCH_SCALE: float = 0.50
const CROUCH_SPEED_MULT: float = 0.70
const CROUCH_LERP: float = 12.0
const CAM_CROUCH_DROP: float = 0.5
const HEADROOM_MARGIN: float = 0.05
...
const CROUCH_KNEE_DEG := 28.0
const CROUCH_TORSO_DEG := -10.0
const CROUCH_ARM_DEG  := 10.0
...
# Track grounded state between frames for landing detection
var _was_on_floor: bool = false

# ============================================================
# JUMP TYPE + AUDIO
# ============================================================
enum JumpType { STAND, WALK, SPRINT }
var last_jump_type: JumpType = JumpType.STAND

# Standing jump audio
var JUMP_STANDING: AudioStream = preload("res://Sounds/StandingJump.wav")

# Walking jump audio variations
var JUMP_WALK_1: AudioStream = preload("res://Sounds/WalkingJump1.wav")
var JUMP_WALK_2: AudioStream = preload("res://Sounds/WalkingJump2.wav")

# Sprinting jump audio variation
var JUMP_SPRINT_1: AudioStream = preload("res://Sounds/RunningJump1.wav")
var JUMP_SPRINT_2: AudioStream = preload("res://Sounds/RunningJump2.wav")

# Per jump type volume in dB (0 dB = normal, negative = quieter, positive = louder)
@export_range(-24.0, 12.0, 0.5) var stand_jump_db: float = 0.0
@export_range(-24.0, 12.0, 0.5) var walk_jump_db: float = 0.0
@export_range(-24.0, 12.0, 0.5) var sprint_jump_db: float = 0.0

# Internal players made in code
var _jump_player: AudioStreamPlayer3D

# Landing audio
var LAND_SOUND: AudioStream = preload("res://Sounds/Landing.wav")
var _land_player: AudioStreamPlayer3D

# ============================================================
# WALKING / SPRINTING AUDIO
# ============================================================
# Looping stone footstep base sound
var WALK_LOOP: AudioStream = preload("res://Sounds/Walking3D.wav")

var _walk_player: AudioStreamPlayer3D
var _sprint_player: AudioStreamPlayer3D
...
func _ready() -> void:
	...
	# Create internal jump audio player
	_jump_player = AudioStreamPlayer3D.new()
	_jump_player.autoplay = false
	_jump_player.volume_db = 0.0
	_jump_player.attenuation_model = AudioStreamPlayer3D.ATTENUATION_DISABLED
	add_child(_jump_player)

	# Create internal landing audio player
	_land_player = AudioStreamPlayer3D.new()
	_land_player.autoplay = false
	_land_player.stream = LAND_SOUND
	_land_player.volume_db = -2.0
	_land_player.attenuation_model = AudioStreamPlayer3D.ATTENUATION_DISABLED
	add_child(_land_player)

	# Create internal walking audio player (normal pace)
	_walk_player = AudioStreamPlayer3D.new()
	_walk_player.autoplay = false
	_walk_player.stream = WALK_LOOP
	_walk_player.volume_db = -4.0
	_walk_player.pitch_scale = 1.0
	_walk_player.attenuation_model = AudioStreamPlayer3D.ATTENUATION_DISABLED
	add_child(_walk_player)

	# Create internal sprinting audio player (faster, a bit louder)
	_sprint_player = AudioStreamPlayer3D.new()
	_sprint_player.autoplay = false
	_sprint_player.stream = preload("res://Sounds/Running3D.wav")
	_sprint_player.volume_db = -2.0
	_sprint_player.pitch_scale = 1.2
	_sprint_player.attenuation_model = AudioStreamPlayer3D.ATTENUATION_DISABLED
	add_child(_sprint_player)
...
func _update_view_nodes() -> void:
	if fp_pivot_yaw:
		fp_pivot_yaw.rotation.y = yaw
	if fp_pivot_pitch:
		fp_pivot_pitch.rotation.x = pitch
...
func _physics_process(delta: float) -> void:
	...
	if Input.is_action_just_pressed("move_jump") and is_on_floor():
		if _crouch_t < 0.5:
			var jump_type: JumpType = JumpType.STAND
			if is_sprinting:
				jump_type = JumpType.SPRINT
			elif wants_move:
				jump_type = JumpType.WALK
			else:
				jump_type = JumpType.STAND

			last_jump_type = jump_type
			_apply_jump(jump_type)
...
# JUMP HELPERS
# ============================================================
func _apply_jump(jump_type: JumpType) -> void:
	match jump_type:
		JumpType.STAND:
			velocity.y = JUMP_VELOCITY
			_play_jump_sound(JumpType.STAND)
		JumpType.WALK:
			velocity.y = JUMP_VELOCITY
			_play_jump_sound(JumpType.WALK)
		JumpType.SPRINT:
			velocity.y = JUMP_VELOCITY * 1.05
			_play_jump_sound(JumpType.SPRINT)

func _play_jump_sound(jump_type: JumpType) -> void:
	...
	match jump_type:
		JumpType.STAND:
			stream = JUMP_STANDING
			vol_db = stand_jump_db
		...
		JumpType.SPRINT:
			vol_db = sprint_jump_db
			if randi() % 2 == 0:
				stream = JUMP_SPRINT_1
			else:
				stream = JUMP_SPRINT_2

	if stream == null:
		return

	_jump_player.stop()
	_jump_player.stream = stream
	_jump_player.volume_db = vol_db
	_jump_player.play()

func _play_landing_sound() -> void:
	if _land_player == null or LAND_SOUND == null:
		return
	_land_player.stop()
	_land_player.play()
